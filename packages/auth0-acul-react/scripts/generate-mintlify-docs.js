 ...
 * from TypeScript source files (React package)
 *
...
function getFiles(dir, exts = ['.ts', '.tsx']) {
  const files = [];
  const extArray = Array.isArray(exts) ? exts : [exts];
  ...
  function walk(currentPath) {
    ...
    for (const entry of entries) {
      ...
      if (stat.isDirectory() && !entry.startsWith('.')) {
        walk(fullPath);
      } else if (stat.isFile() && extArray.some((ext) => fullPath.endsWith(ext))) {
        files.push(fullPath);
      }
    }
  }
...
function parseTypeScriptFile(filePath) {
  ...
  const functions = [];
  const enums = [];
  const exportedItems = new Set(); // Track exported const items

  function visit(node) {
    // Capture variable declarations that are exported (like export const useLoginId = ...)
    if (
      ...
      if (parent.parent && ts.isVariableStatement(parent.parent)) {
        const varStatement = parent.parent;
        if (
          ...
          if (node.name && ts.isIdentifier(node.name)) {
            ...
            functions.push({
              ...
              description: jsDoc || '',
              params: [],
              returns: node.type?.getText() || 'any',
              filePath,
            });
            exportedItems.add(node.name.text);
          }
        }
      }
    }
...
function generateNestedParamFields(properties, allClassNames, allInterfaceNames, indentLevel = 1) {
  ...
  for (const prop of properties) {
    ...
    const objectInUnion =
      !isObjectProp && !isArrayOfObjectsProp ? extractObjectFromUnionType(prop.type) : null;
  ...
  if (type === 'class') {
    ...
    if (item.members && item.members.length > 0) {
      ...
      for (const member of item.members) {
        ...
        // Check if this is a union type that contains an object
        const objectInUnion =
          !isObjectType && !isArrayOfObjectsType ? extractObjectFromUnionType(member.type) : null;
    ...
    if (item.params && item.params.length > 0) {
      for (const param of item.params) {
        ...
        // Check if this is a union type that contains an object
        const objectInUnion =
          !isObjectType && !isArrayOfObjectsType ? extractObjectFromUnionType(param.type) : null;
    ...
    if (isReturnObjectType) {
      ...
      const baseNormalizedType = escapeGenericBrackets(normalizeType(typeWithoutObj));
      const { type: returnsValue, hasLinks: returnsHasLinks } = generateTypeWithLinks(
        typeWithoutObj,
        allClassNames,
        allInterfaceNames,
        baseNormalizedType
...
async function generateDocumentation() {
  ...
  const srcFiles = getFiles(config.srcDir, ['.ts', '.tsx']).filter(
    (f) =>
      !f.includes('.test.ts') &&
      !f.includes('.test.tsx') &&
      !f.includes('.spec.ts') &&
  ...
  // First pass: Parse all files and collect items
