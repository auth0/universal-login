...
The `logout-aborted` screen is used to handle [purpose of the screen].

## ⚛️ React Example

This example demonstrates how to build a React component for the `logout-aborted` screen.
...
1.  **Imports**: We import `useLogoutAborted` and various context hooks from the dedicated `@auth0/auth0-acul-react/logout-aborted` entry point.
2.  **Hooks**:
    *   `useLogoutAborted()`: Provides the core screen object with methods like `submit()`.
