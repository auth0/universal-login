...
export const EmailOTPChallenge: React.FC = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Main hook for screen logic
  const screen = useEmailOTPChallenge();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {};
      await screen.submitCode(payload);
  ...
  return (
    <form onSubmit={handleSubmit}>
      <h1>EmailOTPChallenge</h1>

      {/* TODO: Add form inputs for the 'submitCode' payload */}
