...
describe('MfaWebauthnNotAvailableError Screen', () => {
  ...
  describe('instance hook', () => {
    it('should provide instance hook that returns screen instance', () => {
      ...
      const screenExports = Object.keys(MfaWebauthnNotAvailableErrorScreen);
      const potentialInstanceHooks = screenExports.filter(key => 
        ...
        key !== 'useResend' &&
        key !== 'useMfaPolling' &&
        typeof (MfaWebauthnNotAvailableErrorScreen as any)[key] === 'function'
      );
      ...
      potentialInstanceHooks.forEach(hookName => {
        try {
          ...
          const { result } = renderHook(() => (MfaWebauthnNotAvailableErrorScreen as any)[hookName]());
    ...
    it('should return stable reference across renders for instance hooks', () => {
      ...
      potentialInstanceHooks.forEach(hookName => {
        try {
          const { result, rerender } = renderHook(() => (MfaWebauthnNotAvailableErrorScreen as any)[hookName]());
          if (result.current && typeof result.current === 'object') {
  ...
  describe('submit functions', () => {
    it('should export submit functions if available', () => {
      ...
      submitFunctions.forEach(funcName => {
        expect(typeof (MfaWebauthnNotAvailableErrorScreen as any)[funcName]).toBe('function');
      });
    });
  });
  ...
  describe('utility hooks', () => {
    it('should export utility hooks if available', () => {
      ...
      const utilityHooks = screenExports.filter(key => 
        ...
        key !== 'useErrors' &&
        key !== 'useAuth0Themes' &&
        key !== 'useMfaWebauthnNotAvailableError'
      );
      
      // Each utility hook should be a function
      utilityHooks.forEach(hookName => {
        expect(typeof (MfaWebauthnNotAvailableErrorScreen as any)[hookName]).toBe('function');
      });
    });
    ...
    it('should call submit functions', () => {
      ...
      submitFunctions.forEach(funcName => {
        try {
          ...
          const func = (MfaWebauthnNotAvailableErrorScreen as any)[funcName];
