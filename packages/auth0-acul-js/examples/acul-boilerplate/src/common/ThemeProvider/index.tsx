...
const getThemeValue = (instance: any, paths: string[], defaultValue: any) => {
  for (const path of paths) {
    const value = path
      .split(".")
      .reduce((acc, part) => acc && acc[part], instance);
...
const ThemeProvider: React.FC<ThemeProviderProps> = ({
  ...
  useEffect(() => {
    ...
    // Get primary color with original fallbacks
    ...
    // Get link-specific color ONLY from branding.settings.colors.primary
    const linkSpecificPrimaryColor = getThemeValue(
      screenInstance,
      [
        "branding.settings.colors.primary", // Only check this path
      ],
      primaryColor // Fallback to the main primaryColor if specific one not found
    );
    ...
    // Explicitly set --color-links using the link-specific primary color
    root.style.setProperty("--color-links", linkSpecificPrimaryColor);
  ...
  return <div className="auth0-acul">{children}</div>;
};
