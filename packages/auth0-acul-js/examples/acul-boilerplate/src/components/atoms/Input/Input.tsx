...
const Input = forwardRef<HTMLInputElement, InputProps>(({
  ...
  return (
    <div className="auth0-input-container">
      {label && (
        <label htmlFor={id} className="auth0-input-label">
          {label}
        </label>
      )}
      ...
      <div className="relative">
        ...
        {isPasswordInput && showPasswordToggle && (
          <button
            type="button"
            className="auth0-password-toggle"
      ...
      {helperText && !error && (
        <p className="auth0-helper-text">{helperText}</p>
      )}
    </div>
  );
});
