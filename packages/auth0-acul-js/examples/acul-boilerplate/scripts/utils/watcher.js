...
export const watchAndUpload = async (screenName) => {
    ...
    try {
      ...
      if (lastBuildTime > 0 && timeSinceLastBuild < CONFIG.BUILD_COOLDOWN) {
        ...
        const remainingTime = Math.ceil(
          (CONFIG.BUILD_COOLDOWN - timeSinceLastBuild) / 1000
        );
      ...
      if (buildSuccess) {
        ...
        const configSuccess = await configureAdvancedMode(
          screenName,
          advancedConfig,
          true
        );
        ...
        if (configSuccess) {
          ...
          logger.error(
            "❌ Hot reload failed - screen reconfiguration unsuccessful"
          );
        }
      }
    } catch (error) {
  ...
  const validDirs = watchDirs.filter((dir) => {
    ...
    if (!exists) {
      logger.warn(
        `Directory does not exist: ${path.relative(process.cwd(), dir)}`
      );
    }
    return exists;
  });
  ...
  if (validDirs.length === 0) {
    logger.warn(
      "No valid directories to watch. Make sure the screen and component directories exist."
    );
